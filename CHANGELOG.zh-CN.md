...
- 侧边栏 agent 行（VIM-423）：侧边栏 Sessions 列表中的 Active 会话卡片现在始终按 coding-agent pane 逐行展示 —— 统一的状态徽标（运行中为旋转细环，由 watcher 生命周期与 OSC 9;4 进度取并集驱动；`!` 待处理、`✕` 错误、`✓` 完成、`◦` 空闲）、agent 名称，以及来自通知流的最新任务消息（依次回退到 pane 标签、`working on <目录名>`）。各行归组在以新 `wash-recess` token（覆盖全部六套主题）绘制的凹陷面板中；点击某行会激活会话并聚焦对应 pane。规格文档：[`docs/superpowers/specs/2026-08-05-session-card-agent-rows-design.md`](./docs/superpowers/specs/2026-08-05-session-card-agent-rows-design.md)。
