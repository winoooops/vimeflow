...
- Sidebar agent rows (VIM-423): Active session cards in the sidebar Sessions list now always show one row per coding-agent pane — a universal state chip (spinning ring while running, driven by the watcher lifecycle united with OSC 9;4 progress; `!` needs-you, `✕` error, `✓` finished, `◦` idle), the agent name, and its latest task message from the notification stream (falling back to the pane label, then `working on <dir>`). Rows group inside a tonal recess panel backed by a new `wash-recess` token across all six themes; clicking a row activates the session and focuses that pane. Spec: [`docs/superpowers/specs/2026-08-05-session-card-agent-rows-design.md`](./docs/superpowers/specs/2026-08-05-session-card-agent-rows-design.md).
- Agent-reply capture for Kimi and OpenCode (VIM-293): dispatching a review comment or question to a Kimi or OpenCode session now renders the agent's structured reply in the diff thread, same as Codex and Claude Code. Kimi buffers the main wire's streamed `content.part` text blocks per turn and flushes at `step.end`/`end_turn`; OpenCode's bridge plugin (v3) aggregates assistant text-part snapshots in memory and writes one tail-clamped `assistant.text` record at `session.idle` — user text never reaches disk, and capture is scoped to assistant messages so the dispatched prompt's own example sentinel block can't round-trip as a reply.
- Whole-changelist delegated review (VIM-327): the diff toolbar's Request-review popover gains a "This file / All changes (N)" scope choice (`f`/`a` hotkeys). "All changes" dispatches every file-strip entry — staged + unstaged + untracked, with a partially-staged file contributing both halves — to the reviewing agent as one paths-only request grouped by `git diff` / `git diff --cached`, untracked files annotated for direct reads. Returned findings anchor across all reviewed files under a single nonce; a path present in both halves resolves by range-match with a working-tree tie-break. The changelist snapshot prefetches when the popover opens (Copy stays inside the clipboard's user-activation window), and merge-conflicted files no longer parse into garbage hunk ranges — combined (`diff --cc`) diffs now degrade to file-level anchors everywhere. Spec: [`docs/superpowers/specs/2026-07-13-vim-327-changelist-review-design.md`](./docs/superpowers/specs/2026-07-13-vim-327-changelist-review-design.md).
- In-file diff search (VIM-252, search half): a floating glass magnifier over the diff body opens a modeless glass popup (the unpinned-sidebar recipe from #645) with substring-precise match highlighting painted into Pierre's shadow DOM via the CSS Custom Highlight API, a `current/total` counter, and vim-modal keys — `/` opens, `Esc` closes and clears, `n`/`p` step matches while search is open and revert to file navigation when closed; `r` now refreshes the active diff from the keyboard (the refresh chip shows the shortcut). Matching is case-insensitive, scoped to the selected file, and survives agent-driven refreshes — query, highlights, and active match are preserved and re-painted on every Pierre rebuild. Spec: [`docs/superpowers/specs/2026-07-02-diff-search-design.md`](./docs/superpowers/specs/2026-07-02-diff-search-design.md).
- Built-in native Ghostty terminal runtime for macOS arm64 packages. The app bundles `libghostty-spm` through the SwiftPM bridge plus `ghostty_native_parent.node`, making the parented Ghostty `NSView` the macOS terminal backbone while the Rust sidecar keeps PTY ownership. xterm.js remains the Linux/dev fallback and the safe path when native Ghostty is unavailable.
...
- Runtime theme system: Catppuccin (dark, default), Flexoki (light), Gruvbox Dark, Gruvbox Light, Tokyo Night, and Dracula are now switchable live via the command palette (`:theme <name>`) without reloading the app. Terminals (native Ghostty on macOS, xterm.js fallback), the code editor (CodeMirror), and the diff viewer (Pierre) all re-theme instantly through their respective bridge adapters. All previously hardcoded hex colors have been migrated to semantic CSS-variable tokens defined in `src/theme/themes/*.ts` and applied at runtime by `src/theme/service.ts`. A new ESLint rule `vimeflow/no-hardcoded-colors` and a CSS guard test prevent color regressions. Spec: [`docs/superpowers/specs/2026-06-11-theme-system-design.md`](./docs/superpowers/specs/2026-06-11-theme-system-design.md).
...
- Diff keyboard shortcuts no longer die after pressing the refresh chip or while an agent rewrites the viewed file. In-panel pointerdowns now stop before DockPanel's section-level focus steal, focus lost to silently unmounting elements (Chromium fires no blur on removal) is reclaimed on the next render, deliberate clicks outside the panel are never overridden, and closing the diff search no longer steals focus from the terminal on cleanup-path closes.
