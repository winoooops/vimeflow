...
if [ "$NODE_VERSION" -lt 24 ]; then
    echo "⚠️  Warning: Node.js $NODE_VERSION detected. This project requires Node.js 24+."
...
if [ -f "package.json" ]; then
    ...
    echo "⚠️  package.json not found. Run the initializer agent first."
fi
...
if command -v rustup &> /dev/null; then
    ...
    echo "⚠️  Rust is not installed. It will be needed for Tauri development."
...
if [ -f "package.json" ]; then
    ...
    npm run lint --silent || echo "⚠️  Lint check failed"

    echo "   - Checking format configuration..."
    npm run format:check --silent || echo "⚠️  Format check failed"
fi
