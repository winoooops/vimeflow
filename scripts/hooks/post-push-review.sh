#!/usr/bin/env bash
# PostToolUse hook: after git push or gh pr create, trigger /harness-plugin:github-review.
#
...
# /harness-plugin:github-review skill automatically.
#
...
if echo "$command" | grep -qE '^\s*gh\s+pr\s+create'; then
  echo "ACTION REQUIRED: A pull request was just created. Wait ~90 seconds for the Codex GitHub Action to post its review, then invoke /harness-plugin:github-review to fetch and address any code review findings."
  exit 0
fi
...
echo "ACTION REQUIRED: Code was just pushed. Wait ~90 seconds for the Codex GitHub Action to post its review, then invoke /harness-plugin:github-review to fetch and address any code review findings."
