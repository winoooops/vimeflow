// CONFIGURATION
const float DURATION = 0.4;                // How long the ripple animates (seconds)
const float MAX_RADIUS = 0.08;             // Max radius in normalized coords (0.5 = 1/4 screen height)
const float ANIMATION_START_OFFSET = 0.25;       // Start clear of the cursor during rapid typing
vec4 COLOR = vec4(iCurrentCursorColor.rgb, 1.0);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    float isCursorMove = step(currentCursor.w * 0.35, distance(centerCC, centerCP));
    float isCursorChange = max(isModeChange, isCursorMove);
    ...
    // don't clamp yet; we need to know if it's > 1.0 (finished)
     ...
     if (isCursorChange > 0.0 && isAnimating > 0.0) {
