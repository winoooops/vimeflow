...
**何必在终端和 GUI 之间二选一？两个一起用 —— 无缝地装进同一个窗口。**

面向 AI 编码代理的终端优先工作空间：你的代理 CLI 就住在真正的终端 pane 里，GUI 环绕在它们四周 —— 而不是压在它们头上。
...
每个编码代理 CLI 都爱把自己的状态塞进一行状态栏，看得人眯眼。Vimeflow 则给每个代理一块清清爽爽的面板 —— 模型、上下文窗口，外加实时的**轨迹流（traces）**；自动识别，不需要任何包装命令，照常吃你熟悉的 CLI 就行。轨迹流是一份可滚动的**历史记录，涵盖代理运行过的每一个工具**，按代理整理成语义化轨迹（Claude、Codex、Kimi、OpenCode 各有一套 profile）；哪一步动了工作区，旁边就是一键 **Show diff**。于是你看到的不只是代理此刻在干什么，而是它干过的所有事 —— 还能直接跳到任意一步留下的改动。
...
2. 按 `⌘;` 执行 `:layout` 选择多 pane 布局（或者用 vim 风格的 `:vsplit` / `:split`）。
...
<table>
  <tr>
    ...
    <td width="33%" valign="top"><img src="docs/media/agent-traces.png" alt="代理状态侧栏中的工具调用轨迹" width="100%" /></td>
  </tr>
  <tr>
    ...
    <td valign="top"><b>工具调用与轨迹</b> —— 实时计数，外加一份可滚动、层层堆叠的历史，记录代理触发过的每一个工具。</td>
  </tr>
</table>

### 套餐用量（在 API 允许的范围内）

对于提供了用量接口的代理，面板会在模型旁边盯着你的剩余额度 —— 会话用量和每周用量，随你干活实时往下掉。
...
一个停靠在终端旁边的完整**内联审查界面** —— 而不是把你甩给 `git diff`。diff 由 **Pierre** 的引擎（`@pierre/diffs`）渲染，并随你的工作区主题着色，变更文件列表就在旁边的 pane 里。你可以就地操作 hunk —— **单独暂存或取消暂存一个 hunk**、**丢弃一个 hunk 或整个文件**（会先跟你确认）—— 而且这里的 review 是一场对话，不是独白：**在某一行留下评论，当前正在干活的代理会在同一条评论串里回应你**、改好代码并标记为已解决。想听第二意见？**Request review** 会把这份 diff 交给一位第三方（tertiary）审查者 —— 派发给另一个代理，或者把提示词复制走，带去哪儿都行。无论哪种方式，你都能逐 hunk 走查代理的改动，趁坏改动还在写的时候就把它抓住，全程不用离开窗口。
...
2. 执行 `:tabnew` 新建会话，用 `:goto` 在会话之间跳来跳去。
