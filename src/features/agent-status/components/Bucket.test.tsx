...
test('renders rounded percent, label, and tick marks at 25/50/75', () => {
  render(<Bucket pct={73.6} color="#cba6f7" label="CTX" uid="ctx-test" />)
...
test('label is rendered with mono tracking class for horizontal readability', () => {
  render(<Bucket pct={50} color="#cba6f7" label="CTX" uid="ctx-mono" />)
...
test('two buckets with different ids render independently', () => {
  render(
    <div>
      <Bucket pct={50} color="#cba6f7" label="CTX" uid="ctx-a" />
      <Bucket pct={80} color="#7defa1" label="CACHE" uid="cache-a" />
    </div>
  )
...
test('pct glyph is tinted with bucket color', () => {
  render(<Bucket pct={50} color="#cba6f7" label="CTX" uid="ctx-tone" />)
