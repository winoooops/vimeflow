...
- **Buttons**: always `Button` / `IconButton` / `ToolbarButton` from `@/components`. A raw icon-only `<button>` wrapping a `material-symbols-outlined` glyph is banned by `vimeflow/no-raw-icon-button` — use `IconButton` (or `ToolbarButton` for icon + label). Contract: `docs/design/UNIFIED.md` §5.10–5.12.
