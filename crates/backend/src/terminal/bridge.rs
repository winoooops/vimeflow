...
pub fn generate_bridge_files(
    ...
    let shim_dir_path = dir.join("bin");
    ...
    let claude_shim_content = format!(
        "#!/usr/bin/env bash\n\
         ...
         for dir in \"${{path_parts[@]}}\"; do\n\
           ...
           if [ -x \"$candidate\" ]; then\n\
             exec \"$candidate\" --settings \"$VIMEFLOW_CLAUDE_SETTINGS\" \"$@\"\n\
           fi\n\
         done\n\
    ...
    // prepend the shim dir: user startup files may have already put it later in
    // PATH and then prepended `/usr/local/bin` ahead of it.
    let shell_init_content = format!(
        "# Auto-generated by Vimeflow for session {session_id}\n\
         ...
         if [ -n \"$VIMEFLOW_CLAUDE_SHIM_DIR\" ]; then\n\
           export PATH=\"$VIMEFLOW_CLAUDE_SHIM_DIR:$PATH\"\n\
...
mod tests {
    ...
    fn shell_init_shims_claude_with_settings_path_chars() {
        ...
        let path = format!(
            ...
            files.shim_dir_path.to_string_lossy(),
    ...
    fn zsh_alias_using_env_still_hits_claude_shim() {
        ...
        let files = generate_bridge_files(dir.to_str().unwrap(), "session-zsh-alias")
            .expect("generate bridge files");
        let path = format!(
            "{}:{}",
            fake_bin.to_string_lossy(),
