    ...
    let settings = serde_json::json!({
        ...
        "hooks": {
            ...
            "PreToolUse": [{
                ...
                "hooks": [{
                    "type": "command",
                    "command": signal_hook(serde_json::json!({
...
mod tests {
    ...
    fn generates_bridge_files_in_temp_dir() {
        ...
        assert!(prompt_command.contains("UserPromptSubmit"));
    ...
    #[cfg(unix)]
    #[test]
    fn attention_hooks_run_without_node_on_path() {
        let tmp = tempfile::tempdir().expect("failed to create temp dir");
        ...
        let settings: serde_json::Value =
            serde_json::from_str(&fs::read_to_string(&files.settings_path).expect("settings JSON"))
                .expect("parse settings JSON");
        let commands = [
            settings["hooks"]["UserPromptSubmit"][0]["hooks"][0]["command"]
                .as_str()
                .expect("prompt hook command"),
            settings["hooks"]["Stop"][0]["hooks"][0]["command"]
                .as_str()
                .expect("stop hook command"),
            settings["hooks"]["PermissionRequest"][0]["hooks"][0]["command"]
                .as_str()
                .expect("permission hook command"),
            settings["hooks"]["PreToolUse"][0]["hooks"][0]["command"]
                .as_str()
                .expect("question hook command"),
            settings["hooks"]["StopFailure"][0]["hooks"][0]["command"]
                .as_str()
