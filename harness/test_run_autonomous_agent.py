...
@pytest.mark.parametrize("reason,expected_summary", [
    (
        # Corrupt-stamp case — reason starts with `.feature_list_stamp.json`.
        ".feature_list_stamp.json exists at /tmp/.feature_list_stamp.json "
        "but could not be parsed (corrupt or hand-edited invalid JSON). "
        "Delete the file and re-run the Initializer.",
        ".feature_list_stamp.json exists at /tmp/.feature_list_stamp.json "
        "but could not be parsed (corrupt or hand-edited invalid JSON)",
    ),
    (
        # Missing-stamp case — reason starts with "no .feature_list_stamp.json".
        "no .feature_list_stamp.json found beside feature_list.json. "
        "The feature list was generated by an earlier harness version.",
        "no .feature_list_stamp.json found beside feature_list.json",
    ),
    (
        # Hash-mismatch case — reason embeds "app_spec.md".
        "app_spec.md has changed since feature_list.json was generated "
        "(stamp hash abc123… vs current def456…). "
        "Delete feature_list.json and re-run.",
        "app_spec.md has changed since feature_list.json was generated "
        "(stamp hash abc123… vs current def456…)",
    ),
])
def test_ignore_stale_list_warning_is_readable(
    tmp_path: Path, capsys, reason: str, expected_summary: str
) -> None:
    """The bypass warning must produce a readable parenthetical for each
    of the three staleness cases. Regression guard for the bug where
    `reason.split('.')[0]` returned "" / "no " / "app_spec" because
    STAMP_FILENAME starts with a dot.
    """
    from agent import run_autonomous_agent
    ...
    with patch("agent._make_session", return_value=_StubSession()), \
         patch("agent.run_agent_session", new=AsyncMock(return_value=("continue", ""))), \
         patch("agent.check_stamp_fresh", return_value=(False, reason)), \
         patch("agent.get_pending_features", return_value=[]), \
         patch("agent.print_progress_summary"), \
         patch("agent.AUTO_CONTINUE_DELAY_SECONDS", 0):
    ...
    captured = capsys.readouterr()
    assert f"WARNING: stale feature_list.json detected ({expected_summary})" in captured.out, (
        f"parenthetical summary malformed — got:\n{captured.out}"
    )
    # Explicit guards against the bug's old output.
    assert "detected ()" not in captured.out
    assert "detected (no )" not in captured.out
    assert "detected (app_spec)" not in captured.out
