...
Vimeflow surfaces each agent's 5-hour / weekly rate limits in the agent-status card (`StatusSnapshot.rate_limits` → `AgentStatusEvent` → `useAgentStatus`). For Claude Code and Codex this is a **pure local-file read** — the agent CLI already made whatever network call and persisted the limit numbers into an artifact Vimeflow then parses:
...
| **Codex**       | the **rollout JSONL** — the `token_count` event carries `rate_limits.primary`/`secondary`, which the Codex CLI lifted from API **response headers** and wrote into its own session file (`codex/mod.rs` `latest_account_rate_limits`) | ❌ reads local JSONL           |
...
1. **A — Rust sidecar adapter fetches `/usages`.** A sync HTTP GET on the supervisor's background thread fills `StatusSnapshot.rate_limits`, the same path Claude/Codex already populate.
