...
- No semicolons, single quotes, trailing commas (es5); arrow components; explicit
  return types on exports; `test()` not `it()`; every new file imports
  `{ test, expect, vi } from 'vitest'`; co-located `*.test.tsx`/`*.test.ts`.
...
- **Done:** classes `bg-browser-bar` / `bg-browser-tab-active` resolve.
...
- `electron/browser-pane.ts`: `ipcMain.handle` → resolve active tab's
  `webContents.getURL()` (the committed URL) → open via `shell.openExternal` only if
  scheme is `http(s)`; else no-op. Resolve URL main-side (don't trust the renderer).
...
- **Tests:** `browser-pane.test.ts` — http(s) opens (mock `shell.openExternal`),
  `about:blank`/non-http(s) no-op, pending-nav opens the committed URL;
  `preload.test.ts` — `openExternal` exposed on the bridge.
...
- `electron/browser-pane.ts`: in `installAppShortcutForwarding`'s
  ...
  else `ctrl && !meta`), no `alt`/`shift`, ignore auto-repeat (inline predicate) →
  `preventDefault` + `BrowserWindow.fromId(record.windowId).webContents.send(
  BROWSER_PANE_FOCUS_ADDRESS, { sessionId, paneId })` + focus the app window.
- `electron/preload.ts` · `types.ts` · `browserBridge.ts`: `onFocusAddress(cb)`,
  mirroring `onFocus`.
- **Tests:** `browser-pane.test.ts` — `KeyL`+platform-mod emits for the right
  `{ sessionId, paneId }`; other keys + `Ctrl+L` on mac do not; `preload.test.ts` —
  `onFocusAddress` exposed.
...
- **Tests:** display segments + lock per scheme; click/Enter/Space → `onBeginEdit`;
  `Enter` → `onSubmit(draft)`; `Escape`/blur → `onCancel`; icons `aria-hidden`.
...
- **Tests:** nav buttons `disabled`, no handler fired; open-external calls the bridge
  with the pane ref; grid + `overflow:hidden` present (structure).
...
- `⌘L`: a chrome-root (`[data-browser-pane-id]`) keydown handler + an
...
- **Tests:** update `BrowserPane.test.tsx` — focus border toggles on the container;
  ...
  simulated `focus-address` set `isEditing`; outside-chrome `⌘L` does not); ported
  draft/committedUrl edge cases (idle-sync pause, blur-cancel, redirect); **drop**
  Tab-to-Go/idle-Go assertions, add Enter-submit.

## Task 10 — Manual real-browser verification (spec §6.3)

- In a real Electron build: every Material Symbol (`public`, `merge`, `adjust`,
  `close`, `add`, `arrow_back`, `arrow_forward`, `refresh`, `lock`, `lock_open`,
  `open_in_new`) renders as a glyph, not raw text; the toolbar shows no horizontal
  overflow / no wrap at a representative narrow (quad-layout) pane width.
