...
**Goal:** Replace `BottomDrawer` (Editor + Diff, bottom-only) with a positionable `DockPanel` (Editor / Diff — unchanged tab set, dockable bottom / left / right) that mirrors the handoff prototype.

> **Scope note (2026-05-17):** the prototype's third `Files` tab is dropped from scope — the sidebar already exposes a Files tree (#185). Task 4 (Files tab + DockFilesPanel) is deleted; subsequent tasks renumber.
...
{isDockOpen && (
  <DockPanel
    ...
    // position + onPositionChange added in Task 5
  />
)}
```

Note: the unmount-when-closed flow lands here in Task 2 (not deferred to Task 7). Task 7 adds the `DockPeekButton` rendered alongside the conditional unmount. Until then, the closed state shows nothing — the close button is unreachable from a user standpoint anyway because we can't currently reopen. Task 2.6 below mitigates this with a temporary always-open guard so the test suite can run.
...
const isDragging = false // owned by WorkspaceView's lifted hook from Task 6
const handleMouseDown = (): void => {
  // The resize-handle drag is owned by the lifted hook starting in Task 6;
...
(Task 5 makes the glyph position-aware; Task 3 restyles to the 24×24 button.)

Finally, change the `<section data-testid="bottom-drawer" ...>` to `data-testid="dock-panel"`. This testid migration was previously marked for Task 6, but doing it here in Task 2 makes it the same commit as the prop rename — atomic.
...
Note: `expand_more` is the bottom-dock glyph. Task 5 makes it position-aware (`chevron_left` / `chevron_right` for side docks).
...
## Task 4: Add DockSwitcher subcomponent
...
- [ ] **Step 4.3: Create DockSwitcher.tsx**
...
## Task 5: Mount DockSwitcher in DockPanel header
...
- [ ] **Step 5.1: Write the failing test in DockPanel.test.tsx**
...
- [ ] **Step 5.3: Add the props to DockPanelBaseProps**
...
- [ ] **Step 5.4: Render DockSwitcher in the header**
...
- [ ] **Step 5.5: Wire the new props in WorkspaceView**
...
## Task 6: Make main canvas react to dockPosition
...
- [ ] **Step 6.1: Write the failing test in WorkspaceView.test.tsx**

Append:

```tsx
describe('Dock position layout (Task 6 of #166)', () => {
...
- [ ] **Step 6.3: Introduce the inner flex wrapper in WorkspaceView.tsx**
...
- [ ] **Step 6.4: Add side-dock sizing inside DockPanel.tsx**
...
- [ ] **Step 6.5: Update the WorkspaceView mocks with new layout-related props**

Task 2 already renamed `MockBottomDrawerProps` → `MockDockPanelProps` and set `data-testid="dock-panel"` on the mock root. Task 5 added `position` / `onPositionChange`. After Task 6, the only addition is none new — but verify the mock interfaces in both files now match the live DockPanel signature:
...
Each `MockDockPanelProps` should declare every required prop from `DockPanelBaseProps` (the v1 set: `tab`, `onTabChange`, `onClose`, `bottomHeight`, `onBottomHeightAdjust`, `position`, `onPositionChange`, `selectedFilePath`, `content`). If TypeScript complains, add the missing prop to the mock interface.

- [ ] **Step 6.6: Run all WorkspaceView tests**
...
- [ ] **Step 6.7: Manual verification (xterm fit) — BLOCKING**
...
1. **Preferred (fix in this PR):** Wrap the layout transition with a one-frame defer so xterm sees the new container before measuring:
   ...
   Add a `refit()` method on TerminalPane that calls its FitAddon. Include the call signature in the same commit as Task 6.

2. **Fallback (explicit deferral):** If the rAF approach doesn't resolve the issue, open a new GitHub issue titled `fix(terminal): re-fit xterm after dock-position change` referencing this plan, label `bug`, and add a `// TODO(#NNN)` comment at the dockPosition state-change site. Commit `feat(workspace): main canvas reacts to dockPosition` WITHOUT the bug present — meaning if it ships broken, revert Task 6's WorkspaceView edit and pause the plan. Do not ship a known clipping regression.

- [ ] **Step 6.8: Run lint + type-check**
...
## Task 7: Add DockPeekButton + render when !isDockOpen
...
- [ ] **Step 7.3: Create DockPeekButton.tsx**
...
- [ ] **Step 7.4: Run the new tests — expect green**
...
- [ ] **Step 7.5: Wire DockPeekButton into WorkspaceView**

In the inner-flex IIFE from Task 6, replace:
...
- [ ] **Step 7.6: Add a WorkspaceView-level layout test**
...
| left     | yes  | diff   | diff renders at 40% width left                        |
| left     | no   | —      | DockPeekButton at left edge, chevron_left icon        |
| right    | yes  | editor | editor renders at 40% width right                     |
| right    | yes  | diff   | diff renders at 40% width right                       |
...
gh pr create --title "feat(dock-panel): refactor BottomDrawer → DockPanel + position switcher" --body "$(cat <<'EOF'
## Summary
...
- Files tab intentionally NOT added — sidebar already exposes the file tree (#185)

## Test plan
- [x] Unit: DockPanel.test.tsx (controlled-only contract + 2 tabs + chip styling)
...
- [x] Manual: 9 dock-position × open/closed × tab combinations
...
- §Goal — covered by the 7-task arc (rename → state lift → restyle → switcher → mount → layout → peek).
- §Source of truth (5 references) — referenced inline in Task 4 (DockSwitcher) and Task 7 (DockPeekButton).
- §State domain for `dockPosition` — covered in Task 2 (state lift) + Task 6 (layout reactivity).
- §Scope in-scope items — covered by Tasks 1-7 (Files-tab item dropped per 2026-05-17 scope note).
- §Out-of-scope items — none implemented, all called out in the spec.
- §DockPanel component contract — covered by Tasks 2, 3, 5.
- §DockSwitcher component contract — covered by Task 4.
- §State model + WorkspaceView layout reactivity — covered by Tasks 2, 6.
- §Closed-state peek + edge awareness + aria — covered by Task 7.
- §Test strategy — every "(new)" / "(kept)" / "(dropped)" item from the spec maps to a Task step.
- §Implementation plan — 1:1 with Tasks 1-7.
- §Risks (xterm fit + drag handle gating + visual diff) — Task 6.4 (drag handle gating), Task 6.7 (xterm fit verification), final smoke test (visual diff).
