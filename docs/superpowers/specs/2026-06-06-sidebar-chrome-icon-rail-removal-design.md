...
- **New `SidebarTopBar` (38px).** Uses the sidebar's own surface (`bg-surface-container-low`) with **no bottom divider**, so the bar blends into the sidebar instead of reading as a separate banded strip. (Its 38px height still matches the session-tab bar, which is what keeps the toggle's vertical position shared — see the toggle decision.) The collapse toggle sits at the bar's **left** (`paddingLeft: 12`); Command Palette + Settings sit on the **right** via a `flex: 1` spacer (not margins, so the row survives reflow).
- **Collapse model: instant hide, NOT a drawer slide.** Collapsing snaps the sidebar shell width to `0` **instantly** (no width transition) and marks the shell `inert` + `aria-hidden`; the inner panel's **header + content stay mounted** (clipped to 0 width), so durable non-chrome sidebar state — file-explorer expansion/scroll and an in-progress session rename draft — survives a collapse/expand, consistent with the chrome-only scope. (The **top bar** is gated on `!collapsed`, so it — and its portaled tooltips, which escape the inert shell — unmount on collapse rather than lingering; it carries no durable state. "Preserved" means durable state, not open popovers.) The outer shell remains a 0-width `auto`-track placeholder so `<main>` reclaims with no gutter. This intentionally supersedes VIM-75's animated drawer _slide_ (the only VIM-75 change is slide → instant); see the toggle decision below for why the slide had to go.
...
- **Real shortcut, not the mock's placeholder.** The Command Palette utility is icon-only; its real palette chord — `formatShortcut(COMMAND_PALETTE_SHORTCUT_KEYS)` (`Ctrl+;` / `⌘;`), not the mock's `⌘K` — surfaces in the button's tooltip as a Zed-style shortcut chip.
...
- **`TopBarUtil`** — a compact **icon-only** button (28×28, matching the toggle): fill `rgba(26,26,42,0.6)`, constant border `rgba(74,68,79,0.3)`, **background-only hover** (no border-color change), wrapped in the project `Tooltip` (`placement="bottom"`, no native `title`). A `disabled?` prop renders `aria-disabled` + suppresses `onClick` (Settings stub). Command Palette (`terminal`) shows its shortcut as the tooltip's chip; Settings (`settings`) is the stub. Both carry `aria-label`.
...
- **Spike verification (in-browser):** rail gone; layout `[sidebar | main | activity]`; top bar uses the sidebar surface with no divider; the three buttons are icon-only with background-only hover; toggle `{12,5,28,28}` open (top bar) and collapsed (tab bar, in-flow, first tab at `x≈49`); stable across resize (240/272/420); Settings `aria-disabled`; project tooltip shows "Show sidebar" + `Ctrl+⇧B` (and the Command Palette chord).

## 6. Acceptance criteria

1. No icon rail anywhere; layout is `[sidebar | main | activity]`; `<main>` reclaims the freed 48px.
2. The sidebar top bar (38px, the sidebar's own surface, no divider) carries the collapse toggle on the left and the icon-only Command Palette + Settings on the right of the same row; no separate footer/strip.
...
6. The toggle and both utilities use the project `Tooltip` (no native `title`); Command Palette opens via its icon-only button, with its chord shown in the tooltip; Settings is an `aria-disabled` stub pending #252.
