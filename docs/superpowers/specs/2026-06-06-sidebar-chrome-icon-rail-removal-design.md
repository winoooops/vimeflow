...
- **Collapse model: instant unmount, NOT a drawer slide.** Collapsing snaps the sidebar shell width to `0` and **unmounts the inner panel** (the outer shell stays as a 0-width `auto`-track placeholder so `<main>` reclaims with no gutter). There is **no width transition** — collapse/expand is instant. This intentionally supersedes VIM-75's animated drawer slide: see the toggle decision below for why.
...
- VIM-75's animated drawer slide is intentionally **replaced** by instant unmount (above) — that is in scope, but no other VIM-75 behaviour changes.
...
- **Grid:** `gridTemplateColumns` `48px auto 1fr auto` → `auto 1fr auto`. The sidebar shell sets `width: collapsed ? 0 : var(--workspace-sidebar-width)` with **no transition**, and renders its inner panel (the `Sidebar` + resize handle) only when `!collapsed`.
- **Toggle homes:** open → passed into `SidebarTopBar` (in the `Sidebar` `topBar` slot); collapsed → passed as the `<Tabs leading>` toggle (`data-testid="sidebar-toggle-tabs"`). Both resolve to `(12, 5)` size 28; exactly one is mounted at a time. Both call the same `toggleSidebar`.
- **Focus continuity:** toggling unmounts the active toggle's host (the sidebar on collapse; the `leading` slot on expand), so a post-toggle focus guard restores focus to the now-visible toggle when focus was dropped to `<body>`. The same guard covers the `⌘B` / `Ctrl+⇧B` / `:toggle-sidebar` paths (collapsing while focus is inside the unmounting sidebar → `<body>` → toggle). It fires only on a real toggle transition (not initial mount) and is deferred a frame so it runs after unmount/palette-close settles. (Implementation is a finalization task — see §5.)
...
- **A11y:** the collapsed sidebar's inner panel is unmounted (nothing focusable lingers); the in-flow tab-bar toggle is the focusable collapse control. `SidebarToggle` keeps `aria-expanded` + label swap; tooltips are the project `Tooltip`; the Settings stub is `aria-disabled`.
...
- **Implementation status:** the spike implements the layout, the instant unmount, both in-flow toggle homes, the `leading` slot, and the project tooltips — all verified in-browser. Remaining finalization work: the post-toggle focus guard; deleting `IconRail.tsx` (+ test) and `mockNavigation.ts` (+ test); and the tests below.
- **Testing (finalization, TDD, ≥80% — NOT yet written):**
  ...
  - `WorkspaceView.*.test.tsx` — UPDATE: no icon rail; top-bar toggle when open, tab-bar `leading` toggle when collapsed (both `(12,5)`); sidebar inner unmounts when collapsed; collapse from a shortcut while focus is in the sidebar restores focus to the toggle; palette open path via the top bar.
