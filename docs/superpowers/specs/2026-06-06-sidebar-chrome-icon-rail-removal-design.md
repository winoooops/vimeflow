...
- **Focus continuity:** see §3 — the post-collapse focus guard retargets to the root toggle whenever, after collapse, `document.activeElement` is `null` / `<body>` / inside the inert sidebar; this covers the in-sidebar shortcut and the `:toggle-sidebar` palette path (which closes to `<body>`), and is deferred a microtask so it wins the race with the palette's focus restore.
