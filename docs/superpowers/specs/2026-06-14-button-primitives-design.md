...
- **Consequence for the ratchet (the overlap fix):** the `no-raw-icon-button` rule will also flag the icon buttons *inside* grouped controls (e.g. `DockTab`). VIM-124 therefore does **not** drive the count to 0. It migrates only its in-scope standalone offenders (deleting their disables) and leaves the grouped-control offenders carrying `// eslint-disable-next-line vimeflow/no-raw-icon-button -- VIM-125: grouped control` — a smaller **frozen floor** that VIM-125 zeroes out. The disable comment names the deferring issue so the hand-off is explicit.
...
`tone: 'danger'` overrides text + hover to error tokens (`text-error hover:bg-error/10`) — for destructive icon buttons (e.g. `ReviewCommentRow` delete). Feature-specific accents (the burner's `agent-shell-accent`, browser nav's `agent-browser-accent`) are **not** variants or tones — they are semantic tokens passed through `className` (allowed by `vimeflow/no-hardcoded-colors`), documented as accent exceptions.
...
**Detection — icon-only only (this is the precision fix for codex's false-positive finding):** report a `<button>` JSX element whose meaningful children are **exactly one** element carrying `material-symbols-outlined` in a literal/template `className` — i.e. no non-whitespace `JSXText` and no other element/expression children. Implementation: visit `JSXElement` named `button`; filter its `children` to drop whitespace-only `JSXText`; if the surviving children are a single `JSXElement` whose `className` literal contains `material-symbols-outlined`, report:

> Raw icon-only `<button>` — use `IconButton` from `@/components` (or `ToolbarButton` for icon + label).

Because it requires *icon-only* content, it does **not** fire on file rows (`ChangedFilesList`), the address bar (`BrowserAddressBar`), or menu-item buttons (`ContextMenu`) — those carry text/inputs alongside the icon. It also will not flag the primitives' own files (`ignores: ['src/components/**']`).

**Known limitation (acceptable):** icon buttons whose class comes from a helper (e.g. `DockTab`'s `tabIconClass()`) are not detected, because the `material-symbols-outlined` literal does not sit under the `<button>`. Those are all grouped controls owned by VIM-125, so the miss does not affect VIM-124. The rule is a forward guardrail against new drift, not an exhaustive migrator.

**Scope:** `files: ['src/**/*.tsx'], ignores: ['src/components/**']` — mirrors the Ring 1/Ring 2 `ignores`.

**Ratchet:** when the rule lands (PR1) every existing icon-only offender gets `// eslint-disable-next-line vimeflow/no-raw-icon-button`, count frozen and recorded in `AGENTS.md`, split into **VIM-124 in-scope** (migrated to 0 across PR1–PR3) and **VIM-125-deferred** (grouped-control offenders, tagged `-- VIM-125`, remaining as the frozen floor). Same mechanism as VIM-119's `@floating-ui` ratchet (6 → 0), but it lands at the VIM-124 floor, not 0.
...
Standalone icon-only buttons and toolbar pills. Grouped controls and `SidebarToggle` are out (§2). **PR1's first task is a full offender inventory** (`grep` every `<button>`-with-`material-symbols`), classifying each as: migrate-now (standalone icon-only), toolbar-pill, deferred-grouped (`-- VIM-125`), or row/menu exception (text alongside the icon — not flagged, not migrated). The table below is the expected migrate-now set; the inventory is the contract.
...
- **PR1 — substrate + icon-only.** The offender inventory (§7); `base/button` (`buttonVariants` + `BaseButton`); `Button`; `IconButton`; the `no-raw-icon-button` rule (grandfather **all** offenders, freeze the count, tag grouped ones `-- VIM-125`); migrate the standalone icon-only buttons (HeaderActions, AgentStatusRail, panel header, ActivityEvent copy, Card kebab, BrowserToolbar nav, ReviewCommentRow, PriorityPlus). Each migration deletes a VIM-124 disable.
...
- **Ratchet floor, not 0.** Because the rule legitimately flags grouped-control icon buttons (VIM-125's territory), VIM-124 cannot reach 0. The floor is explicit, the disables are tagged `-- VIM-125`, and the inventory records the split — so "done" for VIM-124 is an audited non-zero count, and VIM-125 inherits a precise remaining list.
