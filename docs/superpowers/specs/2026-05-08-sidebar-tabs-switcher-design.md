...
5. **No behavioral regression** vs the cycle-6 sidebar that ships post-#184. Existing tests stay green where they cover behavior that's unchanged. Tests that assert `FileExplorer` is visible (currently true because it lives in the always-mounted `Sidebar.bottomPane`) must either switch to the FILES tab first via `userEvent.click(getByRole('tab', { name: 'FILES' }))` or move into `FilesView.test.tsx`. Test count is **nondecreasing** vs main. Visual delta vs cycle-6 sidebar: new tabs row at the top of the content slot, no permanent file explorer below sessions, "+ New Instance" button moved from `Sidebar.footer` into the SESSIONS view body.
...
| **CREATE**    | `src/components/sidebar/SidebarTabs.test.tsx`                   | Unit tests: tablist/tab semantics, active styling, onChange firing on click + Enter/Space, aria-controls passthrough, accent-bar presence. ~10 tests; no arrow-key coverage (intentionally — see §4).                                                                                                                                                                                                                                      |
...
| **MODIFY**    | `src/features/workspace/WorkspaceView.test.tsx`                 | Tests asserting FileExplorer visibility now click `getByRole('tab', { name: 'FILES' })` first. New tests: tab switching toggles which view is visible; both views remain mounted across switches (asserted via `getByTestId('files-view')` and `getByTestId('sessions-view')` returning truthy regardless of activeTab — `ByTestId` queries find hidden DOM by default — and the inactive view's wrapper carrying the `hidden` attribute). |
...
export interface SidebarTabItem<TId extends string = string> {
  ...
  id: TId
  /** Display label rendered inside the tab button. Kept short — the
   *  visual sizing (11 px uppercase JetBrains Mono) assumes ≤ 8 chars. */
  label: string
  /**
   * Optional id of the panel this tab controls. When present, the rendered
   * tab gets `aria-controls={controls}`. When absent, the tab still
   * functions as a tab but does not advertise a panel relationship.
   * Callers that need strict tabpanel conformance pass this and add a
   * matching `aria-labelledby` on their panel.
   */
  controls?: string
}
...
Tabs are implemented as plain buttons with WAI-ARIA Tabs **roles** (so screen readers announce them as a tablist), but **without** the strict roving-tabindex / arrow-key navigation requirement. Rationale: most tab UIs in similar apps don't move selection with arrow keys — users click or Tab+Enter, and that's what the keyboard surface here provides too. Strict WAI-ARIA Tabs (roving tabindex + Left/Right/Home/End) is documented in `## Future work` for callers who later need it.

| Concern              | Behavior                                                                                                                                                                                                                                                                                                   |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Tablist container    | A `<div role="tablist" aria-orientation="horizontal" aria-label={...}>`. The default `aria-label="Sidebar tabs"` is overridable via prop.                                                                                                                                                                  |
| Tab buttons          | Each rendered as `<button type="button" role="tab" aria-selected={id===activeId} aria-controls={item.controls}>`. Default `tabIndex` (= `0`) — every tab is in the document tab order so users can `Tab` between them.                                                                                     |
| Activation           | Click → `onChange(tabId)`. `Enter` / `Space` on a focused tab fires the button's default click → `onChange(tabId)`. No custom keyboard handler.                                                                                                                                                            |
| Focus on selection   | Browser default: clicking or pressing Enter/Space on a button focuses it. The consumer's panel-toggle re-render does not steal focus because the active tab DOM stays mounted with the same identity.                                                                                                      |
| Panel relationship   | `aria-controls={item.controls}` is emitted only when the consumer passes it. Skip when not needed; degrades gracefully without breaking tablist semantics.                                                                                                                                                 |
| Inert hidden subtree | The `hidden` attribute on the inactive panel (set in WorkspaceView, not by this component) excludes its DOM from a11y trees and tab order. `SidebarTabs` itself does nothing about this — focus restoration on tab switch is the responsibility of `onChange` callers if needed (see §1 Decision #3 note). |

What's intentionally NOT implemented in v1:

- Roving tabindex (every tab is `tabIndex={0}`, not `0` for active and `-1` for inactive).
- `ArrowLeft` / `ArrowRight` / `Home` / `End` navigation.
- Focus-only-mode (selection on focus): every selection is an explicit click or Enter/Space.

These are tracked in `## Future work` if a strict-WAI-ARIA-Tabs caller arrives.

### Test plan

`SidebarTabs.test.tsx` covers:

1. Renders one button per `tabs` entry, in order.
2. Active tab carries `aria-selected="true"`; inactive tabs carry `aria-selected="false"`.
3. Every tab has the default `tabIndex={0}` (no roving tabindex).
4. Click on a non-active tab calls `onChange(id)` exactly once with the right id.
5. `Enter` keypress on a focused non-active tab calls `onChange(id)`. `Space` does too. (Both via the `<button>`'s default activation; no custom handler asserted.)
6. Click on the already-active tab still calls `onChange(id)` — implementations can opt for "no-op when same id" later, but the contract here is "fire on every click."
7. `aria-controls` is emitted on the tab when `item.controls` is provided; absent otherwise.
8. `aria-label` defaults to `'Sidebar tabs'` and can be overridden via the prop.
9. The active accent bar element is present on the active tab and absent on others (queried by a `data-testid` like `'sidebar-tabs-accent'`).
10. Tablist container carries `role="tablist"` and `aria-orientation="horizontal"`.
...
1. **Initial render** — SidebarTabs is in the DOM with `aria-label="Sidebar tabs"`; SESSIONS tab is `aria-selected="true"`; FILES tab is `aria-selected="false"`.
2. **Both views are mounted on initial render** — `getByTestId('sessions-view')` and `getByTestId('files-view')` both return truthy.
3. **Hidden toggle** — on initial render `getByTestId('files-view')` has the `hidden` attribute, `getByTestId('sessions-view')` does not. After `userEvent.click(getByRole('tab', { name: 'FILES' }))`, the assertions flip: `sessions-view` has `hidden`, `files-view` does not.
...
Tests in `WorkspaceView.{integration,visual,verification,subscription,notifyInfo,command-palette}.test.tsx` get audited per §3 — anything asserting "FileExplorer is in the sidebar" must `userEvent.click(getByRole('tab', { name: 'FILES' }))` first, OR get moved into `FilesView.test.tsx` as a focused test of the lifted wrapper.

### What about strict tabpanel ARIA?

For v1, the tab views are NOT wired with `role="tabpanel"` + `aria-labelledby`. The hidden subtree's exclusion from a11y is enough for screen readers; the tabs row plus `aria-selected` semantics convey the relationship. If a follow-up needs strict conformance, each view's outer `<div hidden={...}>` adds `role="tabpanel" aria-labelledby={tabId}`, and `SIDEBAR_TAB_ITEMS` gets `controls: 'sessions-view'` / `controls: 'files-view'` set. Tracked in §8 Future work.
...
- New SidebarTabs component (src/components/sidebar/SidebarTabs.tsx) — generic, presentational, click+Enter/Space activation, role="tablist"/"tab" + aria-selected.
...
| **Strict WAI-ARIA Tabs (roving tabindex + arrow keys)**                     | Inside `SidebarTabs.tsx`: add `tabIndex={0/-1}` based on focused-tab state; add an `onKeyDown` handler on the tablist for `ArrowLeft` / `ArrowRight` / `Home` / `End`. Update test plan accordingly.   | small PR                   |
| **Tabpanel ARIA conformance** (`role="tabpanel"`, `aria-labelledby`)        | `SessionsView` / `FilesView` outer divs add the role + linked id; `SIDEBAR_TAB_ITEMS` gets `controls: 'sessions-view'` / `controls: 'files-view'`. Component contracts already support this (§4 + §6). | small PR                   |
