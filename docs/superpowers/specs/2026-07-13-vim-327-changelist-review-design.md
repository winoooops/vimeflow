...
- **Dev git-service parity is a prerequisite**: the vite dev middleware maps untracked (`?`) entries to status `'A'` (`vite.config.ts:404-408`) — `entry.status === 'untracked'` never holds there, `getDiff(..., untracked: false)` 404s, and the atomic abort kills the whole changelist review in dev-browser mode. The middleware is updated to emit `'untracked'` (the serde-lowercase value the `GitStatus` type documents) and to serve a zero-hunk `FileDiff` for empty untracked files instead of 404. Desktop (Rust) already conforms.

### Prefetch on popover open (clipboard gesture)

`openPopover()` kicks off the changelist snapshot fetch immediately when the strip is non-empty. The controller holds **one in-flight promise ref with a `.catch` attached at creation** (a discarded rejection is swallowed there, never unhandled); re-opening while it is unresolved **reuses** it (no second 8-worker pool), and a settled promise is dropped so the next open refetches. Closing the popover or picking file scope merely stops consuming it. Rejections only surface when `arm('changelist')` consumes the promise — the atomic-abort notify above.
...
- **Staleness between strip render and click** is accepted by the existing snapshot-at-dispatch semantics (`PendingReviewRequest.diffSnapshot` doc: "immune to edits after dispatch"). No re-validation.
...
- **Panel integration**: changelist request end-to-end — findings land across two files and both halves of a third.
