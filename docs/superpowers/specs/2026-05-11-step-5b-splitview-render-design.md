...
| 3   | Pure render in 5b — no manager mutations, no IPC calls                                                                                                                                                                                                                                                                                                                                  | Smallest possible 5b. The focus ring is read-only from `pane.active`; clicks are no-ops in 5b. Mirrors 5a's data-only PR strategy.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `pane.id`    | Session-scoped layout-slot id (`'p0'`, `'p1'`, …). Doubles as the grid-area name in the layout's CSS Grid template AND as React's per-pane element key inside SplitView.            |
...
if (import.meta.env.DEV) {
  if (session.panes.length > layout.capacity) {
    throw new Error(
  ...
  const activeIdx = session.panes.findIndex((p) => p.active)
  if (activeIdx >= 0 && activeIdx >= layout.capacity) {
    throw new Error(
      `SplitView invariant violation: session ${session.id} active pane ` +
        `at index ${activeIdx} is beyond layout '${session.layout}' ` +
        `capacity ${layout.capacity}`
    )
  }
  // pane.id naming is purely a React key in 5b — the grid-area for slot
  // `i` always comes from the iteration index (`p${i}`), not pane.id —
  // so we don't validate id-vs-slot here. 5c may revisit if pane.id
  // ever needs to address a specific layout slot.
}
