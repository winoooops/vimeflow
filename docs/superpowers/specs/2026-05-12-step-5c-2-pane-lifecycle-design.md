...
1. **`applyAddPane` / `applyRemovePane` reducers** — new pure
   functions in `src/features/sessions/utils/paneLifecycle.ts`. They
   take `(sessions, sessionId, …) → Session[]` (plus a small
   side-info bag for removePane), enforce 5a's invariants
   (`panes.length ≥ 1`, exactly-one-active, materialized fields), and
   return the same `sessions` reference on no-op branches so React
   skips the re-render. Match the precedent set by 5a's
   `applyActivePane` in `utils/activeSessionPane.ts`.
...
3. **`pickNextActivePaneId(panes, closedIdx): string | null`** — new
   pure helper. Returns `panes[closedIdx - 1].id` if it exists, else
   `panes[closedIdx]?.id` (after splice the closed slot's _successor_
   shifts into the slot's index), else `null` (no panes left, which
   `removePane` blocks upstream). Mirrors the "prev index, fall to
   next" convention from editor splits.
4. **`addPane(sessionId)` mutation on `useSessionManager`** —
   spawn a new PTY inheriting the active pane's `cwd`, append a new
   `Pane` with `id='pN'` (next free index in `session.panes`),
   `agentType='generic'`, `active=true`. Maintain exactly-one-active
   by clearing every other pane's `active` flag. Update materialized
   `Session.workingDirectory` + `Session.agentType` from the new
   active pane. Fire `service.setActiveSession(newPtyId)` after the
   commit. Same `pendingSpawns` accounting as `createSession`.
5. **`removePane(sessionId, paneId)` mutation on `useSessionManager`**
   — `service.kill(pane.ptyId)`; on success, splice the pane out,
   auto-shrink `session.layout`, pick the next-active pane if the
   removed pane was active, re-materialize `Session.workingDirectory`
   - `Session.agentType`. Fire `service.setActiveSession(newActivePtyId)`
     if the active rotated. Early-return with `console.warn` if the
     target session would end up with `panes.length === 0` — the
     tab-strip X (`removeSession`) is the correct path for the
     last-pane case (Decision #4 + #10).
...
| File                                                            | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | LOC  |
| --------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| `src/features/sessions/utils/paneLifecycle.ts`                  | Five pure exports: `applyAddPane(sessions, sessionId, newPane)`, `applyRemovePane(sessions, sessionId, paneId, currentLayoutId): { sessions, removedPtyId?, newActivePtyId? }`, `autoShrinkLayoutFor(nextCount, currentLayoutId)`, `pickNextActivePaneId(panes, closedIdx)`, `nextFreePaneId(panes)`. Each function returns the same `sessions` reference on no-op branches.                                                                                                                              | ~140 |
...
| --------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| `src/features/sessions/hooks/useSessionManager.ts`              | (1) `SessionManager` interface gains `addPane(sessionId): void` and `removePane(sessionId, paneId): void`. (2) `addPane` body: validate session; `setPendingSpawns(c => c + 1)`; `await service.spawn({ cwd: activePane.cwd, env: {}, enableAgentBridge: true })`; build new `Pane` via `nextFreePaneId(prev.panes)` + `agentType='generic'`; `flushSync(setSessions(prev => applyAddPane(prev, sessionId, newPane)))`; `registerPending(result.sessionId)`; `restoreDataRef.current.set(session.id, restoreData)`; `service.setActiveSession(result.sessionId)`; `registerPtySession`; `setPendingSpawns(c => c - 1)`. (3) `removePane` body: validate session + pane; early-return with warn if `session.panes.length === 1`; `await service.kill({ sessionId: pane.ptyId })`; on success drop bookkeeping (`dropAllForPty`, `restoreDataRef.delete`, `unregisterPtySession`); `flushSync(setSessions(prev => applyRemovePane(prev, sessionId, paneId, session.layout)))`; if reducer returned a `newActivePtyId` AND `sessionId === activeSessionIdRef.current`, fire `service.setActiveSession(newActivePtyId)`; await `service.reorderSessions(orderIds)` matching `createSession`/`restartSession` pattern. (4) `setSessionActivePane` gains a post-commit `service.setActiveSession(target.ptyId)` IPC call, guarded by `sessionId === activeSessionIdRef.current`. (5) Return both new mutations from the hook. **No changes to `createSession`, `removeSession`, `restartSession`.** | +~120     |
...
| `src/features/sessions/hooks/useSessionRestore.ts`                | Restore path already groups cached PTYs into multi-pane sessions per 5a; 5c-2 does not change persistence shape.                                                                                                                                                                                       |
...
/** Return value carries the side-info `useSessionManager` needs after
 *  React state commits — which PTY was removed (so bookkeeping can be
 *  dropped) and, if the active pane rotated, which new PTY should be
 *  pushed to Rust. Both are optional: removedPtyId is undefined when
 *  the reducer no-op'd (missing session/pane); newActivePtyId is
 *  undefined when the removed pane was not the active one. */
...
- **New:** 4 files, ~470 LOC (tests share ~270 LOC).
- **Modified:** 10 files, ~+417 / -~13 LOC.
- **Total:** ~+887 / -~13, ~900 LOC across ~14 files.

Comparable to 5c-1 (~1006 LOC across 17 files). The pure-reducer split keeps the test bulk in `paneLifecycle.test.ts` (pure, fast) rather than the React-bound `useSessionManager.test.ts`.
...
export const applyAddPane = (
  ...
  // Caller guards against a pane id collision via nextFreePaneId;
  // this branch is defense-in-depth so a programmatic mis-use
  // can't corrupt the panes array.
  if (session.panes.some((p) => p.id === newPane.id)) return sessions
  ...
  return [...sessions.slice(0, idx), updated, ...sessions.slice(idx + 1)]
}
  ...
  (sessionId: string): void => {
    ...
    void (async (): Promise<void> => {
      try {
        ...
        // Read the freshest session for nextFreePaneId — the user
        // may have addPane'd twice in quick succession.
        ...
        flushSync(() => {
          setSessions((prev) => applyAddPane(prev, sessionId, newPane))
        })
...
| File                          | Tests                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `paneLifecycle.test.ts`       | **`applyAddPane`**: appends pane, flips every other pane to inactive, re-materializes `workingDirectory`/`agentType`, no-op on missing sessionId (same reference returned), no-op on pane.id collision. **`applyRemovePane`**: removes pane, splices array, auto-shrinks layout per all (count, prevLayout) pairs, picks next active when removed pane was active, leaves other panes' active flag untouched when removed pane was inactive, returns `removedPtyId` + `newActivePtyId` correctly, no-op on missing session/pane, no-op on `panes.length === 1`. **`autoShrinkLayoutFor`**: 1 → single, 2 from hsplit → hsplit, 2 from vsplit/threeRight/quad → vsplit, 3 → threeRight, 4 → currentLayoutId (defensive). **`pickNextActivePaneId`**: prev exists → prev.id; closing first pane → panes[1].id; closing only pane (panes.length=1) → null (caller never hits this branch but the helper handles it). **`nextFreePaneId`**: empty → 'p0'; ['p0'] → 'p1'; ['p0','p2'] → 'p1'; ['p1','p0','p2'] → 'p3'. |
| `useSessionManager.test.ts`   | **`addPane`**: 2-pane fixture → 3 panes after addPane, new pane is active, `service.spawn` called with active pane's cwd + `enableAgentBridge`, `service.setActiveSession` called with new ptyId when session is active, NOT called when session is inactive. `pendingSpawns` increments + decrements symmetrically (including on spawn failure). Spawn-during-removeSession: kills the orphan. **`removePane`**: 2-pane fixture → 1 pane after, `service.kill` called with closed pane's ptyId, layout auto-shrinks (vsplit → single), active rotates when active pane closed, `service.setActiveSession` fired with new active ptyId. `panes.length === 1` → warn + no-op. `service.kill` rejects → React state untouched. **`setSessionActivePane`**: fires `setActiveSession` when `sessionId === activeSessionId`; does NOT fire when `sessionId` is a different session.                                                                                                                                    |
...
| Risk                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Mitigation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`addPane` race with concurrent `addPane` on the same session** — User clicks `+` twice quickly; both await `service.spawn`; both compute `nextFreePaneId(sessionsRef.current.panes)` against state that hasn't seen the _other_ pending pane yet, so both pick `'p1'` and one clobbers the other inside `applyAddPane` (the collision guard returns `prev` and silently drops the second click).                                                                                              | The collision guard (`session.panes.some(p => p.id === newPane.id) → return prev`) prevents corruption — at worst the user sees one of the two new panes mount and the other's PTY is orphaned in Rust. Mitigation: `addPane`'s post-spawn block re-reads `sessionsRef.current` (already does), and if `nextFreePaneId` returns a colliding id (rare — the reducer's no-op signals it), kill the freshly-spawned PTY and console.warn. Detailed handling deferred to a follow-up unless smoke-testing reveals it's actually reachable in practice (humans cannot click `+` faster than spawn resolves under typical load). |
| **`removePane` race with concurrent `removeSession` on the same session** — User clicks tab-X concurrent with pane-X. removeSession kills every pane; removePane awakes after the kill and finds the session already gone.                                                                                                                                                                                                                                                                      | `removePane`'s post-kill block re-reads `sessionsRef.current` inside `setSessions` via the reducer's `findIndex(s => s.id === sessionId)`; the no-op branch returns `prev`. The kill IPC may be a double-kill (removeSession also killed this pane's ptyId), but `service.kill` is idempotent on the Rust side (drop the cache entry; no-op if already dropped). No state corruption; one extra `console.warn` from removeSession's `kill failed for a pane` branch is the user-visible cost.                                                                                                                              |
