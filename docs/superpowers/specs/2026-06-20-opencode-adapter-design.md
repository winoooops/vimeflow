...
- **Rejected because:** it requires a **novel** long-lived SQLite-poller + JSONL-writer subsystem with bespoke lifecycle, WAL-safety, cursor persistence, and secret-table whitelisting — there is **no existing template** in the codebase for a Rust adapter that _writes_ a derived feed (Kimi/Codex only _read_ files their agent already writes). For an autonomous build that must pass codex review and stay green, the novelty/risk is not worth it in v1. It remains the natural **v2 backfill** path.
