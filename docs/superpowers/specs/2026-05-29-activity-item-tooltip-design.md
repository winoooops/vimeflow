...
- **Result-content extraction exists but is test-only:** claude_code/transcript.rs has `extract_tool_result_content` but calls it **only inside the test-match branch** (line 589, under `if let Some(matched) = call.test_match`) — non-test results (Read / Edit / Grep / plain Bash) are not extracted today. When a test command matches, the captured content (capped at `MAX_TOOL_RESULT_CONTENT_LEN = 256 KiB`, keeping the **tail** `TOOL_RESULT_TAIL_LEN = 64 KiB` for the summary parser) builds a `TestRunSnapshot` for the `TestResults` panel — **not** an ActivityEvent `bashResult` — and never reaches the tooltip. PR2 must extract a *new* small head preview for **every emitted** Claude Code tool-call completion — i.e. each non-orphaned `tool_result` (orphans are dropped before emit at transcript.rs:579, as today); the five Codex-adapter sites default `result_preview` to `None` this round (see D5).
...
- **True edit before/after** (mockup `before[]`/`after[]`). `args` is summarized to `MAX_ARGS_LEN = 100`; `old/new_string` are not forwarded. Edit degrades to file + (when populated) `+N/−M` + (when present) the post-edit snippet from `resultPreview`.
...
- **UTF-8 / marker safety.** Reserve the marker line's bytes **including a separating newline** (`"\n".len() + TOOL_RESULT_TRUNCATED_MARKER.len()`), and truncate the content head at the nearest **char boundary** at or below `MAX_BYTES − reserved` (`str::is_char_boundary` / `char_indices`, mirroring the boundary-safe slicing in `cap_with_head_and_tail`) — never slice mid-`char` (a byte-index slice through a multibyte char panics). Append the marker on its own line, inserting the separating `\n` **only when the head does not already end with one** (no blank line before the marker). The returned string (content head + optional `\n` + marker) stays ≤ `MAX_BYTES`.
...
- **PR2 catch-up stress (R2)**: a long-transcript replay (hundreds of completions) on attach emits bounded previews without stalling the event queue; steady-state frontend memory stays ≤~200 KiB after the 50-slice.
