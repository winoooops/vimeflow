...
| 5   | Outbound serialization via a bounded `tokio::sync::mpsc::Sender<Vec<u8>>` → single writer task → `tokio::io::stdout()` | Avoids the sync-vs-async Mutex tension and caps queued stdout memory. `EventSink::emit_json` is a sync trait method (PR-A §5.2 lock) — it formats the LSP frame into a `Vec<u8>` and uses `tx.try_send(frame)` as the fast path. If the bounded queue is full, it blocks the producer through a bounded send worker until capacity is available, preserving event delivery while applying backpressure; closed queues still return an error. Request handler tasks call async `tx.send(frame).await` so responses also backpressure instead of being silently dropped. A single writer task owns stdout and, on clean shutdown, closes its receiver so long-lived PTY/event sender clones cannot keep the process alive; it then drains already-queued frames before exiting. Rejected: unbounded channel (review-found memory growth under stdout backpressure); drop-on-full events (can lose PTY state); fixed post-EOF writer timeout (can drop queued responses/events); `std::sync::Mutex<BufWriter<Stdout>>` (sync trait fits but blocks across `await` on response paths); `tokio::sync::Mutex` (can't acquire from a sync trait method without `blocking_lock`). |
...
[vimeflow-backend (Tokio multi-thread)]
  ...
  ├── PTY / watcher tasks (owned by BackendState): emit events → StdoutEventSink::emit_json → bounded send/backpressure(frame_bytes)
  │
...
impl EventSink for StdoutEventSink {
    fn emit_json(&self, event: &str, payload: Value) -> Result<(), String> {
        // Format the LSP frame in this thread; try_send is the fast path, and
        // queue saturation backpressures the producer instead of dropping events.
        ...
        send_event_frame(&self.tx, frame, event)
    }
}
```
...
| `StdoutEventSink::emit_json`                  | Build a `(tx, rx)` channel; create the sink with `tx`; call `emit_json("pty-data", json!({...}))`; assert `rx.try_recv()` yields a single `Vec<u8>` whose body parses to `{"kind":"event","event":"pty-data","payload":...}`. Tx closed → `emit_json` returns `Err(...)`. Full bounded queue → producer blocks until capacity is freed, then emits the event frame.                                                                                                                                                                                                                                                                                                                                                                                | n/a (the sink is unit-tested via channels; subprocess tests exercise it end-to-end).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
- `StdoutEventSink` — 100% (success, closed channel, full-channel backpressure).
...
| Bounded stdout queue stalls event producers if the writer is slow                               | `StdoutEventSink::emit_json` blocks the producer only when the queue is full, preserving event order and delivery while bounding queued memory. Request responses use async `send().await`, so accepted requests backpressure in the same way. This can slow PTY readers under stdout pipe backpressure, but avoids both unbounded memory growth and terminal-state divergence from dropped events. |
