...
| 5   | Outbound serialization via a `tokio::sync::mpsc::UnboundedSender<Vec<u8>>` → single writer task → `tokio::io::stdout()` | Avoids the sync-vs-async Mutex tension. `EventSink::emit_json` is a sync trait method (PR-A §5.2 lock) — it formats the LSP frame into a `Vec<u8>` and calls `tx.send(frame)` (sync, non-blocking on unbounded channel). Request handler tasks do the same. A single writer task `while let Some(frame) = rx.recv().await { stdout.write_all(&frame).await }` is the only owner of the actual stdout handle. No locks. Order = channel arrival order, which is the wall-clock order the emitter resolved (same as Tauri's emit). Rejected: `std::sync::Mutex<BufWriter<Stdout>>` (sync trait fits but blocks across `await` on the async write paths; needs `spawn_blocking` per response — overhead + complexity); `tokio::sync::Mutex` (can't acquire from a sync trait method without `blocking_lock` which is awkward inside async contexts).              |
...
└── runtime/
    ...
    └── ipc.rs                        ← NEW (PR-B). The whole IPC layer:
                                          ...
                                          - `pub async fn run<R: AsyncRead + Unpin + Send>(state: Arc<BackendState>, reader: R, tx: UnboundedSender<Vec<u8>>, cancel: CancellationToken) -> Result<(), io::Error>`
                                          - `pub async fn writer_task<W: AsyncWrite + Unpin + Send>(rx: UnboundedReceiver<Vec<u8>>, writer: W)`
```
...
[vimeflow-backend (Tokio multi-thread)]
  ...
  ├── N handle_request tasks: serde_json::from_value → router match → state.method(args) → tx.send(frame_bytes)
  ├── Writer task: while rx.recv().await { stdout.write_all + flush }; sole owner of tokio::io::stdout()
  ├── PTY / watcher tasks (owned by BackendState): emit events → StdoutEventSink::emit_json → tx.send(frame_bytes)
  │
  └── Channel `tokio::sync::mpsc::UnboundedSender<Vec<u8>> → Receiver`: serializes every outbound frame in arrival order
```

`BackendState`, the `tx` clones, and the `CancellationToken` clones flow through `Arc`. No `Mutex` on the writer path — the single writer task is the lock. The writer task exits when all `tx` clones drop (which happens naturally as the reader task exits and request handlers complete or are cancelled). `BackendState`'s internal `Arc<…>` wrappers (PtyState, SessionCache, AgentWatcherState, TranscriptState, GitWatcherState) are unchanged from PR-A.

### New files

| File                                    | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Approx LOC |
| --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| `src-tauri/src/bin/vimeflow-backend.rs` | `fn main()` shim. Parses `--app-data-dir <path>` via a hand-rolled argv loop (no `clap` dep). Initializes `env_logger` (stderr only). Builds the writer-channel: `let (tx, rx) = mpsc::unbounded_channel();`. Spawns the writer task: `tokio::spawn(writer_task(rx, tokio::io::stdout()))`. Builds `BackendState::new(app_data_dir, Arc::new(StdoutEventSink::new(tx.clone())))`. Builds the `CancellationToken`. Calls `runtime::ipc::run(state, tokio::io::stdin(), tx, cancel).await`. When `run` returns, drops the last `tx` clone → writer task exits → state drops → exit 0. | ~70        |
| `src-tauri/src/runtime/ipc.rs`          | The whole IPC layer. Internal layout: `mod frame { read_frame, write_frame, FrameError }`, `mod router { dispatch, per-method-decoder-structs }`, `pub struct StdoutEventSink { tx: tokio::sync::mpsc::UnboundedSender<Vec<u8>> }`, `pub async fn run<R: AsyncRead>(state: Arc<BackendState>, reader: R, tx: UnboundedSender<Vec<u8>>, cancel: CancellationToken)` (the writer task is separate; see bin shim row). Internal tests via `#[cfg(test)] mod tests` exercise the codec and dispatch against `BackendState::with_fake_sink()`.                                           | ~400       |
...
mod frame {
    ...
    pub enum FrameError {
        ...
        BadHeader(String),              // Resyncable; caller logs + scans for next "Content-Length:".
...
impl EventSink for StdoutEventSink {
    fn emit_json(&self, event: &str, payload: Value) -> Result<(), String> {
        // Format the LSP frame in this thread; send the byte-vec to the writer task.
        // Synchronous, non-blocking on an unbounded channel — fits the sync trait shape
        // without any Mutex / blocking_lock awkwardness. The writer task is the sole
        // owner of stdout; ordering is FIFO across responses and events.
...
pub async fn writer_task<W: AsyncWrite + Unpin + Send + 'static>(
    ...
    while let Some(frame) = rx.recv().await {
        if let Err(err) = writer.write_all(&frame).await {
            log::warn!("stdout write failed: {err}");
            break;
        }
...
mod frame {
    ...
    pub async fn read_frame<R: AsyncBufReadExt + Unpin>(
        ...
        //   (continue past) or give up.
        let mut content_length: Option<usize> = None;
        loop {
            let mut line = String::new();
            let n = reader.read_line(&mut line).await.map_err(FrameError::Io)?;
            if n == 0 {
                // EOF.
                ...
                if name_trim.eq_ignore_ascii_case("Content-Length") {
                    match value_trim.parse::<usize>() {
                        ...
                        Err(err) => {
                            ...
                            // Treat as resyncable; reset content_length to None so
                            // a later well-formed line can supersede it.
                            content_length = None;
                        }
                    }
                    continue;
                }
                // Unknown header — LSP allows it, ignore. Don't count it against
                // resync budget (well-formed `Name: Value` is fine).
                continue;
            }
    ...
    #[derive(Debug)]
    pub enum FrameError {
        /// Header line malformed but the codec has advanced past it without
...
pub async fn run<R: AsyncRead + Unpin + Send>(
    ...
    // observes that exit and we return cleanly.
    Ok(())
}
...
async fn main() {
    ...
    // Drop state explicitly so SessionCache::clear_all() runs before the writer
    // task exits — gives a best-effort flush of any pending event/response.
    state.shutdown();
    drop(state);

    // Writer task exits when ALL tx clones drop. BackendState dropping drops
    // the sink Arc, but PTY reader threads and watcher tasks may still hold
    // sender clones (they outlive BackendState because their lifetime is
    // tied to portable-pty's own threads). To avoid blocking exit forever,
    // give the writer a short bounded drain — long enough to flush in-flight
    // responses, short enough that a stuck pty thread can't pin us indefinitely.
    // Process exit reaps any remaining child PTY processes regardless.
...
| `router::dispatch`                       | For each of the 19 production methods (default `cargo test`): build a `BackendState::with_fake_sink()`, drive `dispatch(state, method, params)`, assert the returned `Value` matches the in-process call's serialized return. The cfg-gated 20th method `list_active_pty_sessions` is tested under `cargo test --features e2e-test`. Inject malformed params → expect `Err("params: ...")`. Unknown method → `Err("unknown method: ...")`. | n/a.                                                                                                                                                                                                                                                                                                                                                          |
...
| End-to-end per method (parity)           | n/a.                                                                                                                                                                                                                                                                                                                                                                                                                                       | For each of the 19 production methods (default `cargo test`): spawn `target/debug/vimeflow-backend` (via `env!("CARGO_BIN_EXE_vimeflow-backend")`), pipe one request frame in, read one response frame out, assert semantic parity with the in-process `BackendState::method(...)` call. The 20th method is exercised under `cargo test --features e2e-test`. |
...
- `StdoutEventSink` — 100% (only two paths: success, closed channel).
...
| PTY threads outlive `BackendState::drop`, holding `tx` clones and preventing writer-task exit   | Bounded 200ms timeout on `writer_handle.await` in `main()` (see §2.6). After the timeout, the bin exits 0 and the OS reaps the PTY threads. No data loss in practice because the writer is `tokio::io::stdout()` which is line-buffered; pending events have already been written by the time we drop state.                                                                             |
...
| `tokio::sync::mpsc::UnboundedSender` allows runaway memory growth if the writer is slow         | Unbounded is the right choice for the event hot path (PTY data must not block on a slow stdout consumer); the writer task is normally the fastest part of the pipeline (raw stdout write). If we see growth in profiling, switch to `bounded(N)` and accept event drops on the PTY-data path (which is acceptable — PR-A's Tauri path also drops on emit failure).                       |
...
| Loss of `tx`-send-order vs PTY-data emit-order if multiple tasks emit concurrently              | The writer task processes the channel in FIFO order (`UnboundedReceiver::recv()`), so wall-clock-arrival order is preserved. Per-session ordering (PTY's own task emits its own events in order on its own thread/task) is preserved because each task's `tx.send(...)` calls are serial within that task. Cross-session interleaving is fine — the renderer keys events by `sessionId`. |
...
- **Shutdown:** Electron main closes stdin → bin sees EOF → bin runs the `state.shutdown()` path → bin exits 0 within `~250ms` (200ms writer drain + cleanup overhead). PR-D's supervisor SHOULD wait this window before sending SIGKILL.
