...
| State (after `idx` resolution) | Behavior                                                                                                                                                                                                                                                                                                                                          |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `len === 0`                    | No-op. No `notifyInfo` (the user has no tabs and expects nothing to happen).                                                                                                                                                                                                                                                                      |
...
Argument parsing distinguishes "input looks like a number attempt" (valid or not) from "input is a tab name":

```typescript
const trimmed = args.trim()
// Looks like a number — covers integers, negatives, decimals, leading/trailing zeros.
// Does NOT match 'NaN', 'Infinity', '2abc', or anything containing non-digit characters.
const isNumericForm = /^-?\d*\.?\d+$/.test(trimmed)
const numeric = Number(trimmed)
const isValidPosition =
  isNumericForm && Number.isInteger(numeric) && numeric > 0
```

The classifier matters: without it, `Number('zzz')` and `Number('1.5')` both produce `NaN`, leaving the dispatcher unable to distinguish "user typed an invalid number" (which should emit a numeric-error message) from "user typed a name that has no match" (which should fuzzy-match or emit a name-not-found message). The regex captures the user's _intent_ — anyone whose input is purely digits / sign / decimal is asking for a position; anyone else is asking by name.
...
| `isNumericForm`, valid position (`isValidPosition && numeric <= len`) | `setActiveSessionId(sessions[numeric - 1].id)`. 1-indexed.                                                                                                                                                                                                                                                                                                                                                        |
| `isNumericForm`, valid position but out of range (`numeric > len`)    | ``notifyInfo(`No tab at position ${numeric}`)``.                                                                                                                                                                                                                                                                                                                                                                  |
| `isNumericForm`, otherwise (`0`, negative, decimal `1.5`)             | `notifyInfo('Position must be a positive integer')`. The decimal case is treated as invalid input (not "round to 1") because rounding hides typos.                                                                                                                                                                                                                                                                |
| Not `isNumericForm`, `sessions` empty                                 | `notifyInfo('No tabs to switch to')`.                                                                                                                                                                                                                                                                                                                                                                             |
| Not `isNumericForm`, fuzzy match                                      | Score every tab name with the existing `fuzzyMatch` from `registry/fuzzyMatch.ts`. If the **best** score is `0` (no real match), ``notifyInfo(`No tab matching ${args}`)``. Otherwise jump to the highest-scoring tab. Ties (multiple tabs at the same top score) are broken by **first occurrence in the `sessions` array**, which is the user's tab-strip order — deterministic and matches what the user sees. |

Worth calling out: tokens like `'NaN'`, `'Infinity'`, `'2abc'` all fall to the not-`isNumericForm` branch. They are perfectly valid tab names and the fuzzy matcher will score them against the live tab list — the user can name a tab `NaN` if they want.
