...
function App() {
  ...
  // One-shot migration: drop older cache-display state so the new "compare"
  // default takes hold; older sessions may have `cacheCollapsedStyle: "bucket"`
  // saved which would otherwise override the new default.
  if (persisted._v !== 3) {
    delete persisted.cacheCollapsedStyle
    delete persisted.activityCollapsed
    persisted._v = 3
  }
  ...
  // --- Sidebar collapse (⌘B / Ctrl+B) ------------------------------------
  useEffect(() => {
    const onKey = (e) => {
      if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'b') {
        e.preventDefault()
        updateTweaks({ sidebarCollapsed: !tweaks.sidebarCollapsed })
      }
    }
    window.addEventListener('keydown', onKey)
    return () => window.removeEventListener('keydown', onKey)
  }, [tweaks.sidebarCollapsed, updateTweaks])
  ...
  const aesthetic =
    window.VIMEFLOW_AESTHETICS[tweaks.aesthetic] ||
    window.VIMEFLOW_AESTHETICS.obsidian
  useEffect(() => {
        ...
        >
          ...
          <div style={{ flex: 1, minHeight: 0, display: 'flex' }}>
            ...
            <ActivityPanel
              ...
              cacheCollapsedStyle={tweaks.cacheCollapsedStyle || 'ring'}
            />
          </div>
          ...
          >
            ...
            <span>v0.9.4</span>
            <span style={{ flex: 1 }} />
            {/* Session duration — moved from agent status panel footer so it
                survives when the panel is collapsed. */}
            {activeSession.startedAgo && activeSession.startedAgo !== '—' && (
              <>
                <Icon name="schedule" size={11} style={{ color: '#8a8299' }} />
                <span style={{ color: '#cdc3d1' }}>
                  {activeSession.startedAgo}
                </span>
                <span>·</span>
              </>
            )}
            ...
            {(activeSession.changes?.added > 0 ||
              activeSession.changes?.removed > 0) && (
              <>
                <span>·</span>
