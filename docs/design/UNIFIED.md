...
3. **Scroll zone** — `ToolCallSummary` · `ActivityFeed` (visible title **Traces**, `role=feed`, `aria-label="Agent traces"`, vertical `outline-variant/40` rail, 10 newest + "N earlier", roving-tabindex; the running tool stays **inline in its chronological slot** as a normal trace row — a breathing **presence dot** (`vf-activity-presence-dot`) on the kind icon plus a success-colored `running Xs` timestamp — and an invisible chip placeholder reserves the OK/FAILED row so the feed never shifts when the task completes) · `FilesChanged` · `TestResults`.

Tool traces resolve through a frontend `AgentToolCallProfile`: agent-specific exact-name mappings layer over the canonical Bash/Edit/Glob/Grep/Read/Write base, then dynamic MCP inference, then a visible `meta` fallback. A tool event always retains its raw `tool` and carries a required display `label`. The shared semantic kinds are `edit`, `write`, `read`, `bash`, `grep`, `glob`, `plan`, `wait`, `agent`, `web`, `interaction`, `external`, and `meta`; shared components own their icon/color treatment. Claude Code's public inventory is pinned to its official tools reference and npm `sdk-tools.d.ts`, while provider modules keep compatibility aliases separate from upstream names.
