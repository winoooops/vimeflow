...
3. **Scroll zone** — `ToolCallSummary` · conditional `LiveActionCard` (the running tool lifted into a **NOW** card — bolt + verb + path + diff + LIVE chip; opens the diff on click) · `ActivityFeed` (visible title **Traces**, `role=feed`, `aria-label="Agent traces"`, vertical `outline-variant/40` rail, 10 newest + "N earlier", roving-tabindex) · `FilesChanged` · `TestResults`.
