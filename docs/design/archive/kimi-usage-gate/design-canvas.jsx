...
function DesignCanvas({ children, minScale, maxScale, style }) {
  ...
  React.useEffect(() => {
    ...
    const t = setTimeout(() => {
      window.omelette
        ?.writeFile(DC_STATE_FILE, JSON.stringify({ sections: state.sections }))
        .catch(() => {})
    }, 250)
  ...
  React.useEffect(() => {
    ...
    const zoomAt = (cx, cy, factor) => {
      ...
      if (k !== 1) {
        ...
        marker =
          hit && hit.closest
            ? hit.closest('[data-dc-slot],[data-dc-section]')
            : null
      ...
      if (marker) {
        ...
        const drift =
          marker.getBoundingClientRect().top - (cy + (markerY0 - cy) * k)
...
function DCSection({ id, title, subtitle, children, gap = 48 }) {
  ...
  const allIds = artboards
    .map((a) => a.props.id ?? a.props.label)
    .filter(Boolean)
    ...
    >
      <div style={{ padding: '0 60px' }}>
        <div className="dc-sectionhead" style={{ paddingBottom: 36 }}>
          ...
          />
          {subtitle && (
            <div style={{ fontSize: 16, color: DC.subtitle }}>{subtitle}</div>
          )}
        </div>
      </div>
      <div
  ...
  const onGripDown = (e) => {
    ...
    const peers = Array.from(
      document.querySelectorAll(
        `[data-dc-section="${sectionId}"] [data-dc-slot]`
      )
    )
...
function DCFocusOverlay({ entry, sectionMeta, sectionOrder }) {
  ...
  const scale = Math.max(
    0.1,
    Math.min((vp.w - 200) / width, (vp.h - 260) / height, 2)
  )
