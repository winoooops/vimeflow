...
- **Residual race (documented, accepted in this PR):** Field docstring spells out the TOCTOU window where a notify callback can be in-flight inside `start_or_replace` when `insert` snapshots `claimed_transcript` as `false`, then the callback completes the mutation, and finally the old handle's Drop tears down the entry the callback just claimed. Window is microseconds (between `watcher.watch()` returning and the caller's `register`); manifests only when a status-file event fires AND the notify callback is mid-`start_or_replace` exactly when insert reads the flag. Impact when triggered: new watcher loses transcript tail until next status update (~100ms later) re-establishes it via `maybe_start_transcript`. Closing fully requires sharing `TranscriptState`'s per-session gate with `AgentWatcherState::insert` — a cross-module architectural change out of scope for this PR. Code-review heuristic: when two state stores share a key but no shared synchronization, TOCTOU races are unavoidable without serializing across both stores. The choices are (a) share a lock/gate, (b) make one store the authoritative owner and have the other reference by id, or (c) accept and document the residual race window. (c) is the right answer when the window is microseconds AND the impact is bounded recovery (next event re-establishes correct state), as it is here.
