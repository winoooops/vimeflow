...
### 45. `expect(el.className).toContain('grid-rows-1')` substring-matches `grid-rows-10`/`grid-rows-12` and would silently regress

- **Source:** github-claude | PR #195 cycle 2 | 2026-05-09
- **Severity:** LOW
- **File:** `src/features/workspace/WorkspaceView.visual.test.tsx`
- **Finding:** A test guarding the workspace grid's `grid-rows-1` invariant (added to fix #193's grid-row-auto bug) used `expect(workspace.className).toContain('grid-rows-1')`. `String.prototype.includes` is a substring check: `'grid-rows-10'.includes('grid-rows-1')` is `true`, so a future Tailwind class change to `grid-rows-10` or `grid-rows-12` (valid utilities, completely different layout meaning) would let the assertion pass while the actual layout regression slips through. Same family as #6 / #19 / #38 / #41 / #44 (assertions named for an invariant they don't actually enforce), specialized to substring-match-on-class-string. The other two new assertions in the same PR (`List.test.tsx`, `AgentStatusPanel.test.tsx`) correctly used `toHaveClass()` — the inconsistency was the smell. The author followed the local file's pre-existing style (`expect(workspace.className).toContain('h-screen')` two tests above).
- **Fix:** Switched the line to `expect(workspace).toHaveClass('grid-rows-1')`. `toHaveClass()` from `@testing-library/jest-dom` splits the class string on whitespace and checks exact membership — `grid-rows-10` no longer false-positives. Did NOT drive-by-fix the two pre-existing `toContain('h-screen')` / `toContain('overflow-hidden')` lines in the same describe block (per the PR-scope discipline rule — those were not in the PR's diff, so a fix there would expand scope beyond what review caught). **Code-review heuristic:** when asserting a Tailwind utility is on an element, default to `toHaveClass('foo')` over `toContain('foo')` — the difference is silent until a sibling utility shares a prefix (`grid-rows-1` / `grid-rows-10`, `gap-2` / `gap-20`, `text-sm` / `text-smith` if the latter ever existed). When updating a test that the reviewer only flagged on one line, leave neighboring stylistic-twin lines alone unless they're behind the same CI gate; surface them in the pattern KB instead.
- **Commit:** _(see git log for the cycle-2 fix commit on PR #195)_
...
### 56. Wall-clock sleeps in a poll-loop test create flakes on loaded CI; the structural fix is a test-only signaling channel to wait on real watcher progress

- **Source:** github-claude | PR #302 round 4 | 2026-05-30
- **Severity:** MEDIUM
- **File:** `crates/backend/src/agent/adapter/codex/session_index.rs` L524-553 (the test); refactor surface in same file's `spawn_watch` → `spawn_watch_inner` extraction
- **Finding:** `mtime_change_picks_up_new_thread_name` sequenced its three phases (thread-startup wait → initial-read complete → mtime-change detected after rewrite) through three absolute `std::thread::sleep` calls (50ms + 1100ms + 700ms). The 50ms thread-startup wait was the weakest point: `spawn_watch` returns immediately after `std::thread::spawn`, and on a heavily loaded CI host the spawned thread may not be scheduled within 50ms. If `initial_read` runs DURING the 1100ms window (instead of before it), `on_caught_up`-equivalent state mutations race with the test's write, and the assertion `titles.iter().any(|p| p["title"] == "first")` may fail randomly. Minimum runtime ~1.85s on every run, slowing the whole suite, AND every failure looks like a non-reproducible flake rather than a real bug. Code-under-test had no built-in synchronization hooks, so the test had to rely on wall-clock timing as a proxy for real watcher progress.
- **Fix:** Extracted `spawn_watch` → `spawn_watch_inner(..., sync_tx: Option<Sender<&'static str>>)` private impl. Production `spawn_watch` calls `inner` with `None`; sends are eliminated by the `if let Some(...)` guard with zero overhead in release builds. Added `#[cfg(test)] pub(crate) fn spawn_watch_with_sync(..., sync_tx)` that calls `inner` with `Some(tx)`. Watcher signals two phase tags: `"initial-read"` after the first `read_thread_name` + `try_emit` pass completes (always fires — even when initial read yields no title, so the test knows startup is past), and `"mtime-update"` after every mtime-triggered re-read + `try_emit` pass. Sends are non-blocking and swallow `SendError` so watcher correctness never depends on whether a receiver is alive. Test now uses `spawn_watch_with_sync` + two `sync_rx.recv_timeout(Duration::from_secs(5))` waits — phase-1 confirms initial read done before the write, phase-2 confirms mtime detection completed after the write. Runtime collapsed from ~1.85s minimum to ~0.7s on a typical machine, and the failure mode is now unambiguous (a real deadlock surfaces as a 5s recv timeout, not a "first title missing" assertion failure). Code-review heuristic: when a test of a poll-loop / event-driven module sequences phases via wall-clock sleeps, the structural fix is a test-only signaling seam — NOT just bumping the sleep durations. Sleeps tune the flake probability; signals eliminate it. The `Option<Sender<_>>` parameter pattern is the standard shape because (a) it costs production zero (one branch on `None`), (b) it preserves the production signature for callers that don't need the hook, (c) signals are explicit timing-independent facts the test can assert on directly, and (d) failure modes (real deadlock vs scheduler hiccup) become distinguishable via recv_timeout vs assertion failure.
- **Commit:** _(PR #302 upsource cycle 4 fix commit)_
- **Follow-up:** see #57 — the signal-based refactor fixed scheduler flakiness on the test's side but left a latent flake mode on the FILESYSTEM side. When the test code touches a file twice in quick succession on a 1-second-resolution filesystem (HFS+ pre-APFS, FAT32, some FUSE tmpdirs), the two writes can share a second boundary and produce identical mtimes — defeating the watcher's `current_mtime == last_mtime` change-detection. The fix is independent of #56's scheduler fix.

### 58. Untested branch in a load-bearing post-condition recheck — refactor into a testable helper rather than rely on integration-style fault injection

- **Source:** github-claude | PR #302 round 12 | 2026-05-30
- **Severity:** MEDIUM
- **File:** `crates/backend/src/agent/adapter/base/path_security.rs` (`canonicalize_and_verify_under_root` extraction + 2 new direct tests)
- **Finding:** `ensure_status_source_under_trust_root` performs a pre-check, then `create_dir_all`, then a post-create canonicalize recheck. The post-create recheck is the sole defense against a same-user attacker who plants a symlink between `create_dir_all` returning and the second `fs::canonicalize` call (a TOCTOU window). The pre-check IS tested (a pre-existing intermediate symlink is rejected); the post-create branch was NOT tested — a single-threaded test can't inject a symlink between two consecutive syscalls without fault injection. The code carried a comment explicitly noting the gap and tracking it as a follow-up. If the post-create check has a latent bug — wrong comparison direction, off-by-one in `starts_with`, or an incorrect edge case — a narrow TOCTOU window remains open with no test to detect it.
- **Fix:** Extracted the load-bearing canonicalize-+-`starts_with` logic into a new `pub(crate) fn canonicalize_and_verify_under_root(path, canonical_root, context_label)` helper. Both the pre-check phase AND the post-create recheck phase call it. The helper is now directly testable — a single-threaded test pre-creates a symlink at the target path pointing outside the root, then calls the helper to assert it's rejected with the "escapes trust_root" diagnostic. Covers the EXACT same canonicalize logic the production post-create branch runs, without needing fault injection between `create_dir_all` and `canonicalize`. Added two regression tests: one for symlink-to-outside rejection, one (companion) for in-root symlink acceptance — pins both the security check and the "not over-strict" property.
- **Code-review heuristic:** When a load-bearing code path is documented as "untested because the test would need fault injection between two syscalls", the refactor-to-testable-helper move is usually feasible: identify the boundary-checking computation, extract it as a pure-ish function taking the inputs the boundary case needs, then test the helper directly with pre-constructed inputs that exercise the boundary. This is strictly better than the alternatives (fault injection seam in production code, dropping the test entirely, or "good enough for the threat model" hand-waving). The helper-extraction also tightens the contract — when both pre and post checks call the same function, an accidental divergence between them becomes impossible.
- **Commit:** _(PR #302 upsource cycle 12 fix commit)_

### 57. mtime-change detection test had a latent flake on coarse-resolution filesystems (HFS+ pre-APFS, FAT32, FUSE tmpdirs) — fix by forcing the seed file's mtime backward

- **Source:** github-claude | PR #302 round 7 | 2026-05-30
- **Severity:** LOW
- **File:** `crates/backend/src/agent/adapter/codex/session_index.rs` (`mtime_change_picks_up_new_thread_name`)
...
### 59. Weakened subscription-invariant assertion after prop-boundary refactor

- **Source:** github-claude | PR #352 round 5 | 2026-06-06
- **Severity:** MEDIUM
- **File:** `src/features/workspace/WorkspaceView.subscription.test.tsx` L261-283, `src/features/workspace/components/AgentStatusCard.test.tsx`
- **Finding:** A reference-equality test that proved exactly one `useAgentStatus()` call fed both the sidebar card and the activity panel was retired when the card switched from raw `agentStatus` props to derived scalar props. The replacement assertion (`expect(useAgentStatus).toHaveBeenCalled()`) passes for any call count ≥ 1, so a future change that adds `useAgentStatus()` inside `AgentStatusCard` would silently introduce duplicate Tauri event listeners without failing the suite.
- **Fix:** Added an executable structural guard in `AgentStatusCard.test.tsx`: mock `useAgentStatus` to throw, render the real card in isolation, assert no throw and that the hook was never called. This preserves the single-subscription invariant without requiring a strict once-count assertion that React re-renders would break. Updated the WorkspaceView test comment to point to the new guard.
- **Commit:** same commit as this entry

### 59. Shared mock object mutated directly without cleanup in test

- **Source:** github-claude | PR #368 round 5 | 2026-06-06
- **Severity:** MEDIUM
- **File:** `src/features/editor/components/CodeEditor.test.tsx`
- **Finding:** `mockEditorView` is declared at `describe`-block scope and shared across all tests. The right-click selection-preservation test assigns `mockEditorView.state.selection.ranges = [{ from: 0, to: 5, empty: false }]` directly. `vi.clearAllMocks()` only resets spy call history, not plain object properties, so the mutated selection state persists after the test ends. Future tests or test-order shuffling inherit the non-empty selection and fail for reasons unrelated to their own setup.
- **Fix:** Reset `mockEditorView.state.selection.ranges` to the default empty-selection shape in `beforeEach`, immediately after `vi.clearAllMocks()`. Every test now starts with a known selection state regardless of what prior tests mutated.
- **Commit:** same commit as this entry

### 60. Module-level backend event subscription registry leaks across tests, causing cascading failures when a listener test fails before cleanup
